...
const alert = () => {
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            if (!phone || !email || !district) {
                Swal.showValidationMessage(
                    `Please enter phone, email and district`
                );
            }
        ...
        fetch(
            `https://dms-json-hosting.herokuapp.com/api/userdata/add?phone=+977${result.value.phone}&email=${result.value.email}&district=${result.value.district}`,
            {
                method: "POST",
            }
        )
        ...
        <>
            <div className="grid grid-cols-12 px-6 gap-3 mt-12 ">
                ...
                <div className="col-span-4">
                    ...
                    <button
                        onClick={alert}
                        className="mt-3 py-2 px-4 text-sm font-bold text-center text-white rounded-lg bg-primary-700 sm:w-fit hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                    >
                        Get Notified
                    </button>
                </div>
