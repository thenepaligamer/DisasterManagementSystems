...
export default function UpdateEvents() {
    ...
    useEffect( () => {
        ...
        console.log(token);
        (async () => {
            const response = await fetch(`https://dms-json-hosting.herokuapp.com/api/relief/update/${id}`, {
                method: "GET",
                headers: {
    ...
    async function submit(e){
        e.preventDefault();
        const { province, district, local, rice, date, sugar, readymade, salt, water} = e.target;
        ...
        const response = await fetch((`https://dms-json-hosting.herokuapp.com/api/relief/update/${id}?` + formData), {
            method: "PUT",
            headers: {
                authorization: `Bearer ${token}`,
            }
        ...
        <div className=" mt-5 ">
            ...
            <div className="flex justify-center">
                <form  className="space-y-3 my-3 rounded w-1/4" onSubmit={submit}>
                    ...
                    <div className="flex ">
                        <label htmlFor="date"
                               className="flex align-middle mr-3  text-sm font-medium text-gray-900 ">Date</label>
                    ...
                    <div className="flex ">
                        ...
                        <input type="text" id="rice" name="rice" value={event.rice}
                               onChange={e => setEvent({...event, rice: e.target.value})}
                    ...
                    <input type="text" id="sugar" name="sugar"
                            value={event.sugar}
                           onChange={e => setEvent({...event, sugar: e.target.value})}
                    ...
                    <div className="flex ">
                        ...
                        <input type="text" id="salt" name="salt" value={event.salt}
                                 onChange={e => setEvent({...event, salt: e.target.value})}
                               className="bg-gray-50 border border-gray-300 w-4/5 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5"
                    ...
                    <div className="flex ">
                        ...
                        <input type="text" id="readymade" name="readymade"
                                 ...
                                 onChange={e => setEvent({...event, readymade: e.target.value})}
                               className="bg-gray-50 border border-gray-300 w-4/5 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5"
                               placeholder="Optional" required />
                    </div>
                    <div className="flex ">
                        ...
                        <input type="text" id="readymade" name="water"
                                 value={event.water}
                                 onChange={e => setEvent({...event, water: e.target.value})}
                               className="bg-gray-50 border border-gray-300 w-4/5 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5"
                               placeholder="Optional" required />
                    </div>
                    <div className="flex ">
                        ...
                        <input type="text" id="housing" name="housing"
                                 value={event.housing}
                                 onChange={e => setEvent({...event, housing: e.target.value})}
                               className="bg-gray-50 border border-gray-300 w-4/5 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5"
                               placeholder="Optional" required />
                    </div>
